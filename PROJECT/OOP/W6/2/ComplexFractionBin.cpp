...
void writeFractionToFile(ComplexFraction compFrac, char *filename)
{
    ...
    try
    {
        ...
        // myfile.write(reinterpret_cast<char *>(&compFrac), sizeof(compFrac));
...
void printAllFraction(char *filename)
{
    ...
    while (!myfile.eof())
    {
        try
        {
            myfile.read(reinterpret_cast<char *>(&compFrac), sizeof(compFrac));
            ...
            if (!myfile.eof())
                compFrac.print();
            cout << endl;*/
            cout.write(reinterpret_cast<char *>(&compFrac), sizeof(compFrac));
        }
        catch (runtime_error &ex)
        {
            cout << ex.what() << endl;
...
int main()
{
    ComplexFraction cf1;
    int menuChoice;
    char filename[25] = "ComplexFractions.bin";
    while (1)
    {
